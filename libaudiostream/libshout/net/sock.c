...
int sock_try_connection (int sock, const char *hostname, const unsigned port)
{
    ...
    printf ("sock_try_connection_init\ n");
...
{
    int sock;
    printf ("hostname = %s, port = %d, timeout = %d \ n", hostname, port, timeout);
    sock = socket(AF_INET, SOCK_STREAM, 0);
    printf ("sock= %d\n", sock);
    if (sock == -1)
    {
    ...
    if (timeout)
    {
        ...
        if (sock_try_connection (sock, hostname, port) < 0)
        {
            ...
            printf ("atleast error 342 :: ret = %d\n", ret);
            if (ret <= 0)
            {
    ...
    else
    {
        ...
        if (sock_try_connection (sock, hostname, port) < 0)
        {
            printf ("sock try connection error\n");
            sock_close (sock);
            sock = SOCK_ERROR;
        }
    }
